...
import { Plus, Edit, Trash2, Calendar, Search, Filter, Loader2, Clock, BookOpen } from "lucide-react";
...
function Periodes() {
  ...
  useEffect(() => {
    const fetchPeriodes = async () => {
      try {
        const response = await fetch("http://localhost:5000/api/periodes");
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Êtes-vous sûr de vouloir supprimer cette période ?")) return;
    
    try {
      setDeletingId(id);
      const response = await fetch(`http://localhost:5000/api/periodes/${id}`, {
        method: "DELETE"
      });

      if (!response.ok) throw new Error("Échec de la suppression");

      setPeriodes(prev => prev.filter(periode => periode._id !== id));
  ...
  const filteredPeriodes = periodes.filter(periode =>
    periode.mois?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    periode.ua?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    periode.trimestre?.toLowerCase().includes(searchTerm.toLowerCase())
  );

  const totalPeriodes = periodes.length;
  const uniqueTrimestres = [...new Set(periodes.map(p => p.trimestre))].length;
      ...
      <AdminLayout>
        <div className="flex justify-center items-center h-64">
          <div className="text-center">
            ...
            <p className="text-gray-600 font-medium">Chargement des périodes...</p>
          </div>
        </div>
      </AdminLayout>
    );
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-gray-600 text-sm md:text-base">
              Gérez les périodes scolaires et les unités d'apprentissage
            </p>
          </div>
          <Button
            onClick={() => navigate("/admin/periodes/add")}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <Card className="bg-gradient-to-r from-blue-50 to-blue-100 shadow-lg rounded-2xl border-0">
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-blue-600 font-medium">Total périodes</p>
                  <p className="text-2xl font-bold text-blue-800">{totalPeriodes}</p>
                </div>
          ...
          <Card className="bg-gradient-to-r from-green-50 to-green-100 shadow-lg rounded-2xl border-0">
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-green-600 font-medium">Trimestres</p>
                  <p className="text-2xl font-bold text-green-800">{uniqueTrimestres}</p>
                </div>
          ...
          <Card className="bg-gradient-to-r from-purple-50 to-purple-100 shadow-lg rounded-2xl border-0">
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-purple-600 font-medium">Période actuelle</p>
                  <p className="text-2xl font-bold text-purple-800">{currentMonth}</p>
                </div>
              ...
              <Table className="min-w-full">
                <TableHeader>
                  <TableRow className="bg-[#38bdf8]/10">
                    <TableHead className="min-w-[150px] font-semibold text-[#0a2540]">Mois</TableHead>
                    <TableHead className="min-w-[150px] font-semibold text-[#0a2540]">Unité d'apprentissage</TableHead>
                    <TableHead className="min-w-[120px] font-semibold text-[#0a2540]">Trimestre</TableHead>
                      ...
                      <TableCell colSpan={4} className="text-center py-12">
                        <div className="text-center">
                          ...
                          <p className="text-gray-500 font-medium">
                            {searchTerm ? "Aucune période trouvée" : "Aucune période enregistrée"}
                          </p>
                          {!searchTerm && (
                            <p className="text-gray-400 text-sm mt-1">
                              Commencez par ajouter votre première période
                            </p>
                          )}
                        </div>
                      </TableCell>
                        ...
                        <TableCell className="text-right">
                          <div className="flex justify-end gap-2">
                            <Button
                              ...
                              onClick={() => navigate(`/admin/periodes/edit/${periode._id}`)}
                            >
                              <Edit size={16} />
                            </Button>
                            <Button
                              size="sm"
                              variant="destructive"
                              disabled={deletingId === periode._id}
                              onClick={() => handleDelete(periode._id)}
                              className="hover:bg-red-700 transition-all duration-200"
                            >
                              {deletingId === periode._id ? (
